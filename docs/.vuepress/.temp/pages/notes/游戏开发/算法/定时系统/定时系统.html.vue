...
<h3 id="延时补偿" tabindex="-1"><a class="header-anchor" href="#延时补偿"><span>延时补偿</span></a></h3>
<p>一般在定时器执行延时操作时，都会存在一定的误差，当定时器长期运行后，这种误差会逐渐增大。
这种时候，我们可以通过添加延时补偿来修正定时效果，从而减小累计延时误差。</p>
<p>延时补偿的原理如下图所示
<img src="@source/notes/游戏开发/算法/定时系统/res/pic_01.png" alt="延时补偿原理"></p>
...
<img src="@source/notes/游戏开发/算法/定时系统/res/pic_02.png" alt="定时器循环累计误差"></p>
<p>下图为添加了延时补偿的定时器（延时50ms），可以看到每次调用的误差值基本在可控范围
<img src="@source/notes/游戏开发/算法/定时系统/res/pic_03.png" alt="延时补偿定时器"></p>
<h3 id="定时器驱动" tabindex="-1"><a class="header-anchor" href="#定时器驱动"><span>定时器驱动</span></a></h3>
...
<li>
<p>单一线程驱动：创建一个新的线程，在线程中以固定频率检测定时任务。该方式定时在独立线程中执行，定时精度可控。</p>
</li>
<li>
<p>多线程驱动：为每个任务创建一个线程，每个线程单独执行该定时任务。</p>
</li>
</ul>
